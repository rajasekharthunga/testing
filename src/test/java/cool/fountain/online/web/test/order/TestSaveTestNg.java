package cool.fountain.online.web.test.order;
...
public class TestSaveTestNg extends AbstractTestNGSpringContextTests{
	@Value("${server.url}")
	...
	public void testSave(String apiKey, String apiUri, String requestJsonfile) {

		logger.info("Running saveOrder");
		...
		requestBody.contentType(MediaType.APPLICATION_JSON).syncBody(json).exchange().expectStatus().isOk()
				.expectBody(String.class).isEqualTo("true");

		logger.info("Completed saveOrder");
	}

	@Test(invocationCount = 100, threadPoolSize = 10)
	@Parameters({ "apiKey", "apiUri", "requestJsonfile", "maxResponseTimeThreshold" })
	public void testSaveOrderLoad(String apiKey, String apiUri, String requestJsonfile, Long maxResponseTimeThreshold)
			throws JsonParseException, JsonMappingException, IOException, JSONException, ParseException {
		logger.info("Running testSaveOrderLoad");
		logger.info("responseTimeThreshold: " + maxResponseTimeThreshold);
		Long startTime = System.currentTimeMillis();
		OrderSave(apiKey, apiUri, requestJsonfile);
		Long duration = System.currentTimeMillis() - startTime;
		assertTrue(duration <= maxResponseTimeThreshold,
				"Response time of order/save call more than: " + maxResponseTimeThreshold);
		logger.info("Completed testSaveOrderLoad");
	}

	public void OrderSave(String apiKey, String apiUri, String requestJsonfile) {
		String url = getUrl(apiUri);
		WebTestClient testClient = WebTestClient.bindToServer().baseUrl(url).build();
		String json = TestAutomationUtil.getContent(requestJsonfile);
		RequestBodyUriSpec requestBody = testClient.post();
