package cool.fountain.online.web.test.staff.master;
...
public class TestGetAllStaffForTerminalTestNg extends AbstractTestNGSpringContextTests {
	...
	@Test
	@Parameters({"apiKey", "apiUri","requestJsonfile","numberOfStaff"})
	public void testGetAllStaffForTerminal(String apiKey,String apiUri, String requestJsonfile, int numberOfStaff ) {
		
		logger.info("Running GetAllStaffForTerminal");
		...
		if (statusCode.equalsIgnoreCase("200 OK")) {
			...
			int staffListSize =outputJson.get("staffList.size()");
			if(staffListSize==numberOfStaff) {
				assertTrue(true);
			}
			else {
				assertTrue(false);
		...
		logger.info("Completed GetAllStaffForTerminal");
	}

	@Test(invocationCount = 100, threadPoolSize = 10)
	@Parameters({ "apiKey", "apiUri", "requestJsonfile", "maxResponseTimeThreshold" })
	public void testGetAllStaffForTerminalLoad(String apiKey, String apiUri, String requestJsonfile,
			Long maxResponseTimeThreshold)
			throws JsonParseException, JsonMappingException, IOException, JSONException, ParseException {
		logger.info("Running testGetAllStaffForTerminalLoad");
		logger.info("responseTimeThreshold: " + maxResponseTimeThreshold);
		Long startTime = System.currentTimeMillis();
		getAllStaffForTerminal(apiKey, apiUri, requestJsonfile);
		Long duration = System.currentTimeMillis() - startTime;
		assertTrue(duration <= maxResponseTimeThreshold,
				"Response time of GetAllStaffForTerminal call more than: " + maxResponseTimeThreshold);
		logger.info("Completed testGetAllStaffForTerminalLoad");
	}

	public void getAllStaffForTerminal(String apiKey, String apiUri, String requestJsonfile) {
		String url = getUrl(apiUri);
		WebTestClient testClient = WebTestClient.bindToServer().baseUrl(url).build();
		String json = TestAutomationUtil.getContent(requestJsonfile);
		RequestBodyUriSpec requestBody = testClient.post();
