package cool.fountain.online.web.test.payment.controller;
...
public class TestGetSellerPaymentMethodsTestNg extends AbstractTestNGSpringContextTests{
	...
	public void testGetSellerPaymentMethods(String apiKey, String apiUri, String requestJsonfile) {
		...
		logger.info("Completed GetSellerPaymentMethods");
	}

	@Test(invocationCount = 100, threadPoolSize = 10)
	@Parameters({ "apiKey", "apiUri", "requestJsonfile", "maxResponseTimeThreshold" })
	public void testgetSellerPaymentMethodsLoad(String apiKey, String apiUri, String requestJsonfile,
			Long maxResponseTimeThreshold)
			throws JsonParseException, JsonMappingException, IOException, JSONException, ParseException {
		logger.info("Running testgetSellerPaymentMethodsLoad");
		logger.info("responseTimeThreshold: " + maxResponseTimeThreshold);
		Long startTime = System.currentTimeMillis();
		getSellerPaymentMethods(apiKey, apiUri, requestJsonfile);
		Long duration = System.currentTimeMillis() - startTime;
		assertTrue(duration <= maxResponseTimeThreshold,
				"Response time of getSellerPaymentMethods call more than: " + maxResponseTimeThreshold);
		logger.info("Completed testgetSellerPaymentMethodsLoad");
	}

	public void getSellerPaymentMethods(String apiKey, String apiUri, String requestJsonfile) {
		String url = getUrl(apiUri);
		WebTestClient testClient = WebTestClient.bindToServer().baseUrl(url).build();
		String json = TestAutomationUtil.getContent(requestJsonfile);
		RequestBodyUriSpec requestBody = testClient.post();
