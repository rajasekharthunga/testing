...
public class TestHistoryTestNg extends AbstractTestNGSpringContextTests{
	...
	public void testHistory(String apiKey, String apiUri, String requestJsonfile) {

		logger.info("Running order/history");
		...
		logger.info("Completed order/history");
	}

	@Test(invocationCount = 100, threadPoolSize = 10)
	@Parameters({ "apiKey", "apiUri", "requestJsonfile", "maxResponseTimeThreshold" })
	public void testOrderHistoryLoad(String apiKey, String apiUri, String requestJsonfile, Long maxResponseTimeThreshold)
			throws JsonParseException, JsonMappingException, IOException, JSONException, ParseException {
		logger.info("Running testOrderHistoryLoad");
		logger.info("responseTimeThreshold: " + maxResponseTimeThreshold);
		Long startTime = System.currentTimeMillis();
		history(apiKey, apiUri, requestJsonfile);
		Long duration = System.currentTimeMillis() - startTime;
		assertTrue(duration <= maxResponseTimeThreshold,
				"Response time of OrderHistory call more than: " + maxResponseTimeThreshold);
		logger.info("Completed testtestOrderHistoryLoadLoad");
	}

	public void history(String apiKey, String apiUri, String requestJsonfile) {
		String url = getUrl(apiUri);
		WebTestClient testClient = WebTestClient.bindToServer().baseUrl(url).build();
		String json = TestAutomationUtil.getContent(requestJsonfile);
		RequestBodyUriSpec requestBody = testClient.post();
